...
public class InventoryPanel extends JPanel {
    ...
    private void initComponents() {
        ...
        labelProduct.setText("Inventory");
    ...
    private void calculateProductTotalPages() {
        ...
        try {
            ...
            String sql = "SELECT COUNT(*) FROM ("
                    ...
                    + "GROUP BY size, color, price, gender"
                    + ") AS grouped_count";
    ...
    private void tblProduct(int page, int limit) {
        ...
        String[] columnNames = {"ID", "Name", "Size", "Color", "Price", "Gender", "QTY", "Category", "Description"};
        ...
        try {
            ...
            String sql = "SELECT p.id, p.name, v.size, v.color, v.price, v.gender,"
                    + "SUM(v.qty) AS qty, p.category_id, p.description "
                    + "FROM tblproduct p "
                    + "LEFT JOIN tblproduct_variant v ON p.id = v.product_id "
                    + "GROUP BY p.id, p.name, v.size, v.color, v.price, gender, p.category_id, p.description "
            ...
            while (rs.next()) {
                ...
                data.addRow(new Object[]{id, name, size, color, price, gender, qty, categoryId, description});
            }
    ...
    private void configureTableColumns() {
        ...
        jTableDark.setColumnWidth(2, 50);

        jTableDark.setColumnAlignment(3, JLabel.CENTER);
        jTableDark.setCellAlignment(3, JLabel.CENTER);
        jTableDark.setColumnWidth(3, 100);

        jTableDark.setColumnAlignment(4, JLabel.CENTER);
        jTableDark.setCellAlignment(4, JLabel.CENTER);
        jTableDark.setColumnWidth(4, 80);

        jTableDark.setColumnAlignment(5, JLabel.CENTER);
        jTableDark.setCellAlignment(5, JLabel.CENTER);
        jTableDark.setColumnWidth(5, 50);

        jTableDark.setColumnAlignment(6, JLabel.CENTER);
        jTableDark.setCellAlignment(6, JLabel.CENTER);
        jTableDark.setColumnWidth(6, 80);

        jTableDark.setColumnAlignment(7, JLabel.CENTER);
        jTableDark.setCellAlignment(7, JLabel.CENTER);
        jTableDark.setColumnWidth(7, 80);
    }
